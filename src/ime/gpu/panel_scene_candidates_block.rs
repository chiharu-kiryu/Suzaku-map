{
            ...
            if !chrome.next_token_candidates.is_empty() {
                ...
                let mut chip_x = panel_x + 6.0 * responsive_scale;
                let mut row = 0;
                ...
                for (index, token) in chrome.next_token_candidates.iter().take(6).enumerate() {
                    ...
                    let chip_w = if collapsed_daily_mode {
                        ((token.chars().count() as f32 * 10.0).max(54.0)
                            + if index == 0 { 30.0 } else { 20.0 })
                            * responsive_scale
                    } else {
                        ((token.chars().count() as f32 * 12.0).max(60.0) + 20.0)
                            * responsive_scale
                    };
                    ...
                    if row >= if collapsed_daily_mode { 1 } else { 2 } {
                        break;
                    }
                    let rect = [
                        chip_x,
                        chip_y + row as f32 * if collapsed_daily_mode {
                            0.0
                        } else {
                    ...
                    let layout = TextBlock {
                        ...
                        pixel_size: if collapsed_daily_mode {
                            chip_px * 0.95
                        } else {
                            chip_px
                        },
                    ...
                    chip_x += chip_w + if collapsed_daily_mode {
