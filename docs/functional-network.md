...
- **开发**：审计时工作区新增/增强，现纳入 0.5.6/0.5.7 源码，不能当作 0.5.5 安装包的行为。
...
| F31 | 点击输入框进入独立编辑 → 文本/光标/选择/组合事件 → 面板自己的候选 → 确认目标后发送 | Linux 通过原生宿主文本通道发送；目标是面板/设置自己时阻止；交付失败保留草稿，不把本地提交当作外部成功；开发版快速新手势可继续选词/回退，候选耗尽也保留待撤销入口，见 [N17/N18][completion-audit] | 接通＋0.5.7 修复：[键盘编辑][keyboard]、[面板组合][composition]、[发送控制][controller]、[文本输出][text-output] |
| F32 | 鼠标/触摸笔迹 → 采样、限界、平滑/分组、启发式识别 → 选择结果回填草稿 | 可撤销笔画/清空；不是通用 OCR；独立联想可附带笔迹摘要，不上传笔迹图像；原生回填失败保留笔迹来源；开发版统一光标两侧分隔并保留插入位置，见 [N06][tools-audit] | 受限＋0.5.7 修复：[手写交互][handwriting]、[识别与候选预览][panel-support]、[面板模型配置][composition] |
| F33 | 语音工具 Listen / 样例 → 平台识别接口、权限状态、转写稳定判断 → 手动或启用的自动回填 | Linux C 桥仍是模拟链路；回填不直接提交；开发版绑定采集目标，换框/失焦/断连后停止并保留转写，手动采用仍可用；拒绝后不自动重试，见 [N05/N06][tools-audit] | 受限＋0.5.7 修复：[语音交互][voice]、[语音适配][voice-host]、[Linux 模拟桥][speech-c] |
| F34 | 翻译工具选择源/目标语并点 Translate → 独立工作线程与翻译提示 → 八语译文分页预览 | 不依赖自动联想开关；仅发当前可见草稿，不读剪贴板或选区；需要模型，没有离线词库翻译回退；质量取决于模型；开发版目标/授权与磁盘冲突时要求重载、不发送，见 [N15][model-audit] | 接通、模型受限＋0.5.7 修复：[翻译控制][translation-panel]、[翻译接口][translation]、[翻译提供器][model-translation] |
...
| F42 | 面板/设置/托盘修改联想开关或温度 → 单次在途补丁、宿主落盘并确认 → 两窗口与托盘协调 | 状态刷新不是写入确认；失败回退未再次修改的控件；不忙循环重发；开发版保留首次状态前的显式选择，配置冲突时拒绝并提示重载，见 [N07/N08][settings-audit] | 接通＋0.5.7 修复：[设置同步][prediction-settings]、[宿主控制][host]、[托盘][tray] |
