...
一个会在升级后过期的宿主。升级后先退出面板，再重启 `suzaku-ibus.service` 和面板；
有输入正在进行时请先完成或取消组合。
...
本机 Llama/Ollama 服务和模型需另行配置，包不附带模型，也不会自动下载权重。
...
配置上限 64 KiB、备份上限 256 KiB；拒绝特殊文件和最终路径为符号链接的备份/配置。
...
# 以上只预览，没有写入；检查两项变化后：
# 1. 从托盘正常退出 Suzaku（释放托盘拥有的输入法激活）
# 2. 若宿主已注册为用户服务：
systemctl --user stop suzaku-ibus.service
# 3. 明确应用：
suzaku-tool data restore ./my-settings.json --apply
# 4. 之前运行过的服务和面板可重新启动：
systemctl --user start suzaku-ibus.service
suzaku-panel
```
